...
prefs-provider-chatgpt-web = ChatGPT 网页版
...
prefs-web-summary-chatgpt-mode-hint = 扩展会在发送前尝试切换网页端的 智能 模式，并兼容旧版 Instant / Thinking 入口。
...
prefs-web-summary-log-level = Zotero 日志级别
prefs-web-summary-log-level-off = 关闭
prefs-web-summary-log-level-error = 仅错误
prefs-web-summary-log-level-debug = 调试
prefs-web-summary-log-level-hint = 控制网页版总结在 Zotero 端的日志输出。默认仅输出错误，排查问题时再切到调试。
error-profile-not-found = 未找到配置
