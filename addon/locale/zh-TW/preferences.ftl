...
ainote-prefs-webSummarySection = 網頁版AI設定
...
prefs-provider-chatgpt-web = ChatGPT 網頁版
prefs-web-summary-enable = 啟用網頁版總結入口
prefs-web-summary-enable-hint = 開啟後會在 Zotero 條目右鍵選單中顯示「使用網頁版大模型總結」。
prefs-web-summary-auto-start-bridge = 啟動時自動啟動本地 Bridge
prefs-web-summary-auto-start-bridge-hint = 外掛啟動後自動監聽 127.0.0.1 本地連接埠，供瀏覽器擴充套件領取任務。
prefs-web-summary-enable-continue-chat = 啟用「繼續對話」右鍵選單
prefs-web-summary-enable-continue-chat-hint = 僅對已記錄會話映射的文獻顯示，用於開啟對應的 ChatGPT 歷史對話。
prefs-web-summary-bridge-port = Bridge 連接埠
prefs-web-summary-bridge-port-hint = 預設 23123。瀏覽器擴充套件中的 Bridge URL 需要與此保持一致。
prefs-web-summary-chatgpt-mode = ChatGPT 模式
prefs-web-summary-chatgpt-mode-hint = 擴充套件會在送出前嘗試切換網頁端的 智能 模式，並相容舊版 Instant / Thinking 入口。
prefs-web-summary-project-url = ChatGPT 專案 URL
prefs-web-summary-project-url-hint = 填入完整專案頁面連結，例如 https://chatgpt.com/g/g-p-xxxx/project。外掛會優先主動開啟此連結。
prefs-web-summary-poll-interval = 輪詢間隔（毫秒）
prefs-web-summary-poll-interval-hint = Zotero 輪詢本地 Bridge 任務狀態的時間間隔。
prefs-web-summary-timeout = 本地請求逾時（毫秒）
prefs-web-summary-timeout-hint = Zotero 存取本地 Bridge 的單次逾時限制。
prefs-web-summary-log-level = Zotero 日誌等級
prefs-web-summary-log-level-off = 關閉
prefs-web-summary-log-level-error = 僅錯誤
prefs-web-summary-log-level-debug = 除錯
prefs-web-summary-log-level-hint = 控制網頁版總結在 Zotero 端的日誌輸出。預設僅輸出錯誤，排查問題時再切到除錯。
error-profile-not-found = 找不到設定檔
