...
prefs-provider-chatgpt-web = ChatGPT Web
...
prefs-web-summary-chatgpt-mode-hint = The extension will try to switch the ChatGPT web UI intelligence mode before sending, while remaining compatible with the legacy Instant and Thinking entries.
...
prefs-web-summary-log-level = Zotero log level
prefs-web-summary-log-level-off = Off
prefs-web-summary-log-level-error = Errors only
prefs-web-summary-log-level-debug = Debug
prefs-web-summary-log-level-hint = Controls web-summary logs on the Zotero side. Default is errors only; switch to debug when diagnosing issues.
